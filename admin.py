...
class admin:
        ...
        def requestsearch_command(self):
            ...
            self.button_requestdelete = Button(self.frame, text='Complete Request', command=self.requestcomplete_command)
            self.button_requestdelete.place(x=200, y=360,width=100,height=40)

        def requestcomplete_command(self):
            backend.request_delete(self.title_text.get(),self.isbn_text.get())
        ...
        def delete_command(self):
            backend.delete(self.isbn_text.get())

        def update_command(self):
            self.listbox.delete(0,END)
            backend.update(backend.get_id(self.isbn_text.get()),self.title_text.get(),self.author_text.get(),self.year_text.get(),self.isbn_text.get())
            self.listbox.insert(END,(self.title_text.get(),self.author_text.get(),self.year_text.get(),self.isbn_text.get()))
