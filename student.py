...
class student:
        def __init__(self,window):
            ...
            self.button_return = Button(self.frame, text="Return",command=self.return_command)
            self.button_return.place(x=600, y=320,width=100,height=40)
        ...
        def request_command(self):
            backend.request_insert(self.title_text.get(),self.author_text.get(),self.isbn_text.get())
            self.listbox.delete(0,END)
        ...
        def issue_command(self):
            if (len(self.title_text.get())==0):
                messagebox.showinfo("Title Missing","Please Enter Book name..")
            elif (len(self.author_text.get())==0):
                messagebox.showinfo("Author name Missing","Please Enter Author name to issue a book")
            else :       
                backend.issue_insert(self.title_text.get(),self.author_text.get())
        ...
        def return_command(self):
            backend.check_return(self.title_text.get(),self.author_text.get())
